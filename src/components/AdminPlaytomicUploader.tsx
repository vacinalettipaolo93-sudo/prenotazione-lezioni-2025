...
function simpleCsvParse(text: string): Record<string, string>[] {
  // parser CSV minimale:
  // - assume separatore virgola
  // - supporta campi racchiusi da doppie virgolette
  // - ritorna array di righe come dizionari header->value
  const lines: string[] = [];
  let curr = '';
  let inQuotes = false;
  ...
  const handleApply = async () => {
    ...
    try {
      if (onCreateBlocks) {
        ...
        // default: POST JSON to createEndpoint
